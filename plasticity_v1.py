...
class Plasticity(Problem):
    ...
    def custom_init(self):
        ...
        self.fe = self.fes[0]
        self.epsilons_old = np.zeros((len(self.fe.cells), self.fe.num_quads, self.fe.vec, self.dim)) # check this, but I'm assuming num_quads is the number of cells, which are quadrilaterals
    ...
    def compute_avg_stress(self):
        # For post-processing only: Compute volume averaged stress --> summing the stress over each cell and dividing by the total volume
        ...
        vol = np.sum(self.fe.JxW) # what is this? the volume? would make it a lot easier
...
# doesn't work rn. because jax doesn't allow me to set a higher-than-1D array
# def center(point):
#     # find center nodes
#     center_coords = np.array([[Lx/2], [Ly/2], [0.]])
#     return np.isclose(points, center_coords, atol = 1e-2)
...
disps = np.linspace(0., -0.1, 11) # no stacking needed for only compression

location_fns = [[center], bottom, top]
value_fns = [[dirichlet_val_bottom]*4, get_dirichlet_top(disps[0])] # initial bcs
vecs = [[0,1,2], 2, 2]

            # location funcs  # which coords are fixed?  # with what value are they fixed?
dirichlet_bc_info = [location_fns, vecs, value_fns]
# shape is [function list, integer list, function list] 
...
for i, disp in enumerate(disps):
    ...
    dirichlet_bc_info[-1][-1] = get_dirichlet_top(disp) # set new displacement top
    problem.fe.update_Dirichlet_boundary_conditions(dirichlet_bc_info) # update Dirichlet
    # what does sol_list contain that you need to specify index 0? 
    sol_list = solver(problem, solver_options={'petsc_solver': {}}) # solve the problem
